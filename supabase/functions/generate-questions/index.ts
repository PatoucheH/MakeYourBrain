  ...
  try {
    ...
    for (const theme of themes) {
      ...
      while (questionsAdded < TARGET_QUESTIONS && attempts < MAX_ATTEMPTS) {
        ...
        console.log(`Attempt ${attempts}: Generating ${questionsToGenerate} questions for ${themeName}`)
...
- Generate UNIQUE and DIVERSE questions, avoid common/obvious questions`
            }]
          })
        })
        ...
        for (const q of generated.questions) {
          try {
            ...
            if (existingQuestion) {
              console.log(`Question already exists: "${q.question_en.substring(0, 50)}..."`)
              continue
            }
            ...
            console.log(`✓ Added question ${questionsAdded}/${TARGET_QUESTIONS}`)
      ...
      console.log(`Finished ${themeName}: ${questionsAdded}/${TARGET_QUESTIONS} new questions added`)
    }
