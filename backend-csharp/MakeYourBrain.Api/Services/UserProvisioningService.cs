...
public class UserProvisioningService(DapperConnectionFactory connectionFactory)
{
    public async Task ProvisionAsync(Guid userId, string email, string displayName, string preferredLanguage = "en")
    {
        using var conn = connectionFactory.CreateConnection();

        // auth.users stub keeps FK constraints (user_stats / user_lives / user_profiles → auth.users) satisfied
